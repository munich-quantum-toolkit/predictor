...
def test_train_and_qcompile_with_hellinger_model(source_path: Path, target_path: Path, device: Target) -> None:
    ...
    with warnings.catch_warnings():
        ...
        rl_predictor.train_model(timesteps=5, test=True, seed=0)
        ...
        if sys.platform == "win32":
            with pytest.warns(
                ...
                match=f"{re.escape('Setting the shape on a NumPy array has been deprecated in NumPy 2.5.')}|{re.escape('Timeout is not supported on Windows.')}",
            ):
